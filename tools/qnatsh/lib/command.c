			  ...
			  struct cmd_element **cmd)
{
  ...
  enum match_type match = 0, last_match = 0;
  int varflag;
  char *command;
  ...
  for (index = 0; index < vector_active (vline); index++)
	...
	if (vty->node == TEST_NODE && match == extend_match)
	{
		if (index == vector_active (vline) - 1)
		{
			int wrongcmd = 0;
  ...
  while(*tmp != (char)0)
  	{
  		if ((*tmp == '\'') || (*tmp == '"'))
		{
			vty_out (vty, "Host cannot contain \' and \".%s", VTY_NEWLINE);
      		return CMD_WARNING;
		}
		tmp++;
  	}
...
extern int execute_command (const char *command, int argc, const char *arg1, const char *arg2, const char *arg3, const char *arg4);
       ...
       "Reset system's network name\n")
{
  if (host.name)
    XFREE (MTYPE_HOST, host.name);
  host.name = NULL;
  execute_command("rm", 2, "-f", HOSTNAME_PATH, NULL, NULL);
  execute_command("rm", 2, "-f", "/opt/work/nsh/hostname", NULL, NULL);
  return CMD_SUCCESS;
}
