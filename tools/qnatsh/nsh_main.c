...
char config_default[] = VTYSH_DEFAULT_CONFIG;
//char config_default[] = "";
char history_file[MAXPATHLEN];
char *dia_passwd = NULL;
...
vtysh_signal_init ()
{
  ...
  vtysh_signal_set(SIGALRM, sigalarm);
  vtysh_signal_set(SIGHUP, sighup);
...
int command_timeout = 300;//second
...
vtysh_rl_gets ()
{
  ...
  if ((vty->node == AUTH_NODE) || (vty->node == TEST_AUTH_NODE))
	{
...
#define DEFAULT_CONF_FILE    "/etc/qnat/qnatcfg.conf"
extern int load_priv_conf(char *config_default_dir);
...
main (int argc, char **argv, char **env)
{
  ...
  //char *path = "PATH=/usr/local/bin:/bin:/usr/bin:/usr/local/sbin:/usr/sbin:/sbin";
  ...
  while (1) 
    {
      opt = getopt_long (argc, argv, "be:c:d:nEhCP", longopts, 0);
  ...
  if (boot_flag)
    {
      if (vtysh_read_config (DEFAULT_CONF_FILE))
	{
	...
	exit (0);
	  ...
	  confp = fopen (DEFAULT_CONF_FILE, "r");
	  if (confp != NULL){
	    fclose(confp);
  		load_priv_conf(DEFAULT_CONF_FILE);
	  }
  	}
  ...
  vtysh_readline_init ();
  host.motdfile = "/etc/qnat/logo.conf";
  vty_hello (vty);
  ...
  //alarm(command_timeout);
	...
	{
		char cmd[256] = {0};
		int lastnode = vty->node;
		vtysh_rl_gets();
		if(line_read) {
			ret = vtysh_execute (line_read);
		}
        else
        {
                if(ttyname(0) == NULL)
