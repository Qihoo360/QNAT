...
extern void fill_bypass_dest(struct ether_addr *eth_addr, struct netif_port *port, struct rte_mbuf *mbuf);
#endif
...
#ifdef CONFIG_NETOPS_BYPASS_ROUTING
			fill_bypass_dest(&neighbour->eth_addr, neighbour->port, m);
#endif
            neigh_fill_mac(neighbour, m);
            netif_xmit(m, neighbour->port);
            return EDPVS_OK;
