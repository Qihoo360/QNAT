...
static void log_current_time(void)
{
    ...
    RTE_LOG(INFO, CFG_FILE, "load qnat configuation file at %s\n", buf);
}
...
extern char blk_log_file[];

static void blk_log_file_handler(vector_t tokens)
{
    char *log_file = set_value(tokens);
    assert(log_file);
    RTE_LOG(INFO, CFG_FILE, "blk_log_file = %s\n", log_file);
    strncpy(blk_log_file, log_file, strlen(log_file));
    FREE_PTR(log_file);
}
...
void install_global_keywords(void)
{
	...
	install_keyword("blk_log_file", blk_log_file_handler, KW_TYPE_NORMAL);
}
