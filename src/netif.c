...
/* Note: Lockless, lcore_conf is set on initialization stage by cfgfile /etc/qnat/qnat.conf.
config sample:
static struct netif_lcore_conf lcore_conf[NETIF_MAX_LCORES + 1] = {
    {.id = 1, .nports = 2, .pqs = {
...
static void print_kni_sent2kern_loop_debug(struct netif_queue_conf *qconf, 
					   unsigned int pkt_num)
{
    ...
    char dst_buffer[128];
    struct rte_mbuf *mbuf;
    RTE_LOG(WARNING, NETIF, "%s: qconf->kni_len: %d, pkt_num: %d\n",
                    __func__, qconf->kni_len, pkt_num);
    for(index=0; index<(qconf->kni_len - pkt_num); index++){
        mbuf = qconf->kni_mbufs[index];
        struct ipv4_hdr *ip4h = ip4_hdr(mbuf);
        sprintf(src_buffer, "%u.%u.%u.%u",
            (unsigned char )*((char *)&ip4h->src_addr + 0),
            (unsigned char )*((char *)&ip4h->src_addr + 1),
            (unsigned char )*((char *)&ip4h->src_addr + 2),
            (unsigned char )*((char *)&ip4h->src_addr + 3));
        sprintf(dst_buffer, "%u.%u.%u.%u",
            (unsigned char )*((char *)&ip4h->dst_addr + 0),
            (unsigned char )*((char *)&ip4h->dst_addr + 1),
            (unsigned char )*((char *)&ip4h->dst_addr + 2),
            (unsigned char )*((char *)&ip4h->dst_addr + 3));
        RTE_LOG(WARNING, NETIF, "%s: src_ip: %s, dst_ip: %s\n",
            __func__, src_buffer, dst_buffer);
    }
}
...
static void kni_send2kern_loop(uint8_t port_id, struct netif_queue_conf *qconf)
{
    ...
    if (qconf->kni_len > 0) {
        if (kni_dev_exist(dev)) {
            rte_spinlock_lock(&kni_lock);
	    #if 0
	    RTE_LOG(WARNING, NETIF, "%s: Before: kni_tx_burst:: qconf->kni_len: %d, pkt_num: %d\n",
                    __func__, qconf->kni_len, pkt_num);	
	    #endif
            pkt_num = rte_kni_tx_burst(dev->kni.kni, qconf->kni_mbufs, qconf->kni_len);
	    #if 0
	    RTE_LOG(WARNING, NETIF, "%s: After: kni_tx_burst:: qconf->kni_len: %d, pkt_num: %d\n",
                    __func__, qconf->kni_len, pkt_num);
	    #endif
            rte_spinlock_unlock(&kni_lock);
        } else {
        ...
        if (unlikely(pkt_num < qconf->kni_len)) {
            RTE_LOG(WARNING, NETIF, "%s: fail to send pkts to kni\n", __func__);
	    print_kni_sent2kern_loop_debug(qconf, pkt_num);
            free_mbufs(&(qconf->kni_mbufs[pkt_num]), qconf->kni_len - pkt_num);
        }
        qconf->kni_len = 0;
    }
...
static inline int port_name_alloc(portid_t pid, char *pname, size_t buflen)
{
    ...
    if (pid >= phy_pid_base && pid < phy_pid_end) {
        ...
        RTE_LOG(ERR, NETIF, "%s: not enough ports configured in qnat.conf\n", __func__);
        return EDPVS_NOTEXIST;
...
inline static void netif_port_init(const struct rte_eth_conf *conf)
{
    ...
    if (nports_cfg < nports)
        rte_exit(EXIT_FAILURE, "ports in DPDK RTE (%d) != ports in qnat.conf(%d)\n",
                nports, nports_cfg);
