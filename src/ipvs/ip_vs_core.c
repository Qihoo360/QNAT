...
extern int bypass_routing(int af, uint16_t proto, struct rte_mbuf *mbuf);
extern int direct_bypass_routing(const union inet_addr *saddr, struct rte_mbuf *mbuf);
extern inline bool is_local_pkt(uint32_t daddr);
#endif
                    ...
                    const struct inet_hook_state *state)
{
    ...
    if (!conn) {
#ifdef CONFIG_NETOPS_BYPASS_ROUTING
		if(likely(!is_local_pkt(iph.daddr.in.s_addr)))
		{
			if(direct_bypass_routing((union inet_addr *)&iph.saddr, mbuf) == INET_STOLEN)
				return INET_STOLEN;
		}
		if(bypass_routing(af, iph.proto, mbuf) == INET_STOLEN)
			return INET_STOLEN;
#endif
                    ...
                    const struct inet_hook_state *state)
{
    ...
    /* filter kni ip route in case of src ip snated */
    rt = route4_input(mbuf, (struct in_addr *)&iph.daddr.in, (struct in_addr *)&iph.saddr.in,
                      iph.proto, netif_port_get(mbuf->port));
    if (rt && (rt->flag & RTF_KNI)){
	route4_put(rt);
        return INET_STOP;
    }
    route4_put(rt);
