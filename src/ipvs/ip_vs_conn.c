...
static inline void conn_dump(const char *msg, struct dp_vs_conn *conn)
{
    ...
    RTE_LOG(WARNING, IPVS, "%s [%d] %s %s:%u %s:%u %s:%u %s:%u refs %d\n",
            msg ? msg : "", rte_lcore_id(), inet_proto_name(conn->proto),
            daddr, ntohs(conn->dport), caddr, ntohs(conn->cport),
            vaddr, ntohs(conn->vport), laddr, ntohs(conn->lport),
...
extern void *ns_con_inc(uint32_t sip);
extern void ns_con_dec(void *snode);
extern int ns_conn_allowed(uint32_t sip);
#endif
...
static void conn_expire(void *priv)
{
    ...
    if (rte_atomic32_read(&conn->refcnt) == 1) {
        ...
        this_conn_count--;
#ifdef CONFIG_NETOPS_CONN_THRESH
		ns_con_dec((struct ns_sip_stat_node *)conn->con_stat);
#endif
...
void blk_log(const char *format, ...);

unsigned long last_log_ts = 0;
char blk_log_file[128] = {0};
...
void blk_log(const char *format, ...)
{
	...
	int i;
        ...
        if(now == last_log_ts)
                return ;
        if(NULL == blkfp)
                blkfp = fopen(blk_log_file, "a+");
        if(blkfp)
        {
                asct = asctime(localtime(&now));
		memset(ts, 0, 32);
		if(asct)
		{
                	memcpy(ts, asct, strlen(asct));
			for(i = 0; i < 32; i++)
			{
				if(ts[i] == '\n')
					ts[i] = '\0';
                                   ...
                                   struct dp_vs_dest *dest, uint32_t flags)
{
...
#ifdef CONFIG_NETOPS_CONN_THRESH
	if(unlikely(!ns_conn_allowed(ip4_hdr(mbuf)->src_addr)))
	{
		blk_log("source address "NIPQUAD_FMT" blocked for exceed threshhold\n",
			NIPQUAD(ip4_hdr(mbuf)->src_addr));
		return NULL;
	}
#endif
    ...
    this_conn_count++;
#ifdef CONFIG_NETOPS_CONN_THRESH
	new->con_stat = ns_con_inc(ip4_hdr(mbuf)->src_addr);
#endif
    /* schedule conn timer */
...
extern int bp_conn_init_timeout;

static void bypass_conn_init_timeout_handler(vector_t tokens)
{
    char *str = set_value(tokens);
    int init_timeout;
    ...
    init_timeout = atoi(str);
    if (init_timeout > IPVS_TIMEOUT_MIN && init_timeout < IPVS_TIMEOUT_MAX) {
        RTE_LOG(INFO, IPVS, "bypass_conn_init_timeout = %d\n", init_timeout);
        bp_conn_init_timeout = init_timeout;
    } else {
        RTE_LOG(WARNING, IPVS, "invalid bypass_conn_init_timeout %s, using default %d\n",
                str, 30);
        bp_conn_init_timeout = 30;
    }
...
#ifdef CONFIG_NETOPS_BYPASS_ROUTING
    install_keyword("bypass_conn_init_timeout", bypass_conn_init_timeout_handler, KW_TYPE_NORMAL);
#endif
