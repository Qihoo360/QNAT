...
#if 0
static void print_ipv4_rcv_fin_debug(struct rte_mbuf *mbuf)
{
    int af = AF_INET;
    char src_buffer[128];
    char dst_buffer[128];
    ...
    src_ip = inet_ntop(af, &saddr, src_buffer, 
		sizeof(src_buffer)) ? src_buffer : "::";
    dst_ip = inet_ntop(af, &daddr, dst_buffer, 
		sizeof(dst_buffer)) ? dst_buffer : "::";
    RTE_LOG(WARNING, IPV4, "%s: src_ip: %s, dst_ip: %s\n",
            __func__, src_ip, dst_ip);
}

static void print_rt_info_debug(struct route_entry *rt)
{
	char *src_addr = NULL;
	src_addr = inet_ntoa(rt->src);
	char *gw_addr = NULL;
	gw_addr = inet_ntoa(rt->gw);
	char *dst_addr = NULL;
	dst_addr = inet_ntoa(rt->dest);
	RTE_LOG(WARNING, IPV4, "%s: flag =%d, src = %s, dst = %s, gw = %s\n", 
		__func__, rt->flag,src_addr, dst_addr, gw_addr);
}
#endif
...
static int ipv4_rcv_fin(struct rte_mbuf *mbuf)
{
    ...
    } else if (rt->flag & RTF_DEFAULT){
		if (etype != ETH_PKT_HOST) {
            /* multicast or broadcast */
            route4_put(rt);
            RTE_LOG(DEBUG, IPV4, "%s: RTF_DEFAULTRTF_DEFAULT\n", __func__);
            return EDPVS_KNICONTINUE; /* KNI may like it, don't drop */
        }
		return ipv4_output(mbuf);
	}else {
